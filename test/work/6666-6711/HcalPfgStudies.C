...
int main(int argc, char* argv[])
{
  ...
  << "Totally used: " << (double)(endTime - startTime)/CLOCKS_PER_SEC/60.0 << " mins" <<std::endl; 
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  ofstream out_tdc("out_invalidTDC.txt");
  ofstream out_tsf("out_ts2fraction.txt");
...
//          std::cout << "Test information: QIE10DigiLETDC[0][1]= " << QIE10DigiLETDC->at(0).at(1) << std::endl;
    ...
    {
      endTime = clock();
      float elapsed = (float)(endTime - startTime)/CLOCKS_PER_SEC/60.0;
      if ( flag == 0 )
        std::cout << "Processing event " << jentry+1 << "/" << nentries << ". Have completed:  " << (int)job_percent << "% of the task..." << std::endl;
...
//  if (run == 297224 && ls >= 20 && ls <= 23) continue;
    ...
    */
    ...
    for (unsigned int ch = 0; ch < QIE10DigiIEta->size(); ++ch) {
      ...
      if (ts2_charge_fraction > 1)
      {
        std::cout << "Find large TS2 charge fraction!" << std::endl;
        std::cout << "TS2 charge fraction: " << ts2_charge_fraction << std::endl;
        std::cout << "iEta is: " << QIE10DigiIEta->at(ch) << std::endl;
        std::cout << "iPhi is: " << QIE10DigiIPhi->at(ch) << std::endl;
        std::cout << "Depth is: " << QIE10DigiDepth->at(ch) << std::endl;
        std::cout << "QIE10 # is: " << ch << std::endl;

        out_tsf << "Find large TS2 charge fraction!" << std::endl;
        out_tsf << "TS2 charge fraction: " << ts2_charge_fraction << std::endl;
        out_tsf << "iEta is: " << QIE10DigiIEta->at(ch) << std::endl;
        out_tsf << "iPhi is: " << QIE10DigiIPhi->at(ch) << std::endl;
        out_tsf << "Depth is: " << QIE10DigiDepth->at(ch) << std::endl;
        out_tsf << "QIE10 # is: " << ch << std::endl;
        out_tsf << std::endl;
      }
    ...
    std::cout << "QIE10 # is: " << ch <<std::endl;
	  ...
	  out_tdc << "Invalid TDC!" << std::endl;
	  out_tdc << "run " << run << ", ls " << ls << ", evn " << event << ", bx " << bx << ", orbit " << orbit << std::endl;
	  out_tdc << "QIE10 channel " << "(" << QIE10DigiIEta->at(ch) << ", " << QIE10DigiIPhi->at(ch) << ", " << QIE10DigiDepth->at(ch) << ")" << std::endl;
	  out_tdc << "TS" << ts << std::endl;
	  out_tdc << "ADC = " << QIE10DigiADC->at(ch).at(ts) << ", TDC = " <<  QIE10DigiLETDC->at(ch).at(ts) << std::endl;
    out_tdc << "QIE10 # is: " << ch <<std::endl;
    out_tdc << std::endl;
	}
...
//  outputname = "hcal_histograms_rerun.root";
  TFile file_out("hcal_histograms_rerun.root","RECREATE");
